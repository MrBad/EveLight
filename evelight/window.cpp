...
bool Window::Init()
{
    if (SDL_Init(SDL_INIT_EVERYTHING) < 0) {
        std::cerr << "Cannot init SDL: " + std::string(SDL_GetError());
        return false;
    }
    ...
    if (!(mWindow = SDL_CreateWindow(
        ...
        std::cerr << "Cannot create window: " + std::string(SDL_GetError());
        return false;
    }
    if (!(mGLContext = SDL_GL_CreateContext(mWindow))) {
        std::cerr << "Cannot create GL context: " + std::string(SDL_GetError());
        return false;
    }
    if (glewInit() != GLEW_OK) {
