...
struct RenderBatch
{
  uint verticesOffs, numVertices; // offset in vertices buffer, number of vertices
  ...
  RenderBatch(uint verticesOffs, uint indexOffs, DrawType drawType, GLuint textureId)
      : verticesOffs(verticesOffs), numVertices(0), indexOffs(indexOffs), numIndexes(0),
        drawType(drawType), textureId(textureId){}
